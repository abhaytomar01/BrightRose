...
const STYLE_OPTIONS = [
  ...
  { label: "Shirt & Trousers Set", value: "shirt-trousers-set" },
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        ...
        if (p.sizeStock) setSizeStock(p.sizeStock);
        if (Array.isArray(p.tags)) setTags(p.tags);
        if (Array.isArray(p.images)) setOldImages(p.images);
        ...
        <div>
          ...
          {sizes.length > 0 && (
            <div className="mt-4 p-4 border rounded bg-gray-50">
              <label className="block font-medium mb-2 text-sm text-gray-700">Per-Size Stock (Optional)</label>
              <div className="flex gap-4 flex-wrap">
                {sizes.map((s) => (
                  <div key={s} className="flex flex-col">
                    <span className="text-xs text-gray-500 mb-1 uppercase font-semibold text-center">{s}</span>
                    <input
                      type="number"
                      min="0"
                      value={sizeStock[s] ?? ""}
                      onChange={(e) => setSizeStock(prev => ({...prev, [s]: Number(e.target.value)}))}
                      className="border p-2 rounded w-16 text-center shadow-sm"
                      placeholder="-"
                    />
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-500 mt-2">Leave blank to use total stock. If set, this overrides total stock for this size.</p>
            </div>
          )}
        </div>
