...
const AllProducts = () => {
  ...
  const rows = products.map((item) => {
    // Calculate total stock if sizeStock exists
    let displayStock = item.stock || 0;
    if (item.sizeStock && Object.keys(item.sizeStock).length > 0) {
      displayStock = Object.values(item.sizeStock).reduce((acc, curr) => acc + (curr || 0), 0);
    }
