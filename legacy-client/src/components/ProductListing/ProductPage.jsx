...
export default function ProductDetails() {
    ...
    try {
      ...
      if (res.data.action === "added") {
        ...
        trackEvent('AddToWishlist', {
          ...
          content_ids: [String(productIdToToggle)],
  ...
  return (
    <>
      ...
      <div className="max-w-8xl mx-2 px-0 pt-14 md:pt-20 pb-36 bg-white text-[#1A1A1A]">
        {/* PRODUCT MAIN */}
          ...
          <div className="flex flex-col gap-6 px-2 pr-6">
            ...
            <div className="mt-2">
              ...
              <div className="flex gap-2 flex-wrap mt-2">
                {ALL_SIZES.map((s) => {
                  const isSizeListed = Array.isArray(product.sizes) && product.sizes.length ? product.sizes.includes(s) : true;
                  
                  // Check if out of stock based on sizeStock
                  const sizeStockObj = product.sizeStock || {};
                  const availableQuantity = sizeStockObj[s] !== undefined ? sizeStockObj[s] : (product.stock || 0);
                  const available = isSizeListed && availableQuantity > 0;
                  ...
                  if (!available) {
                    return (
                      <button
                        key={s}
                        type="button"
                        className="relative px-3 py-1.5 rounded-md border text-xs text-neutral-400 border-neutral-100 cursor-not-allowed flex flex-col items-center justify-center bg-neutral-50/50 min-w-[55px]"
                      >
                        <span className="opacity-40 font-medium">{s}</span>
                        <span className="text-[7px] leading-none uppercase mt-0.5 opacity-60">Sold out</span>
                      </button>
                    );
                  }
              ...
              {selectedSize && (
                <div className="mt-2 h-4">
                  {(() => {
                    const sizeStockObj = product.sizeStock || {};
                    const stockForSize = sizeStockObj[selectedSize] !== undefined ? sizeStockObj[selectedSize] : (product.stock || 0);
                    if (stockForSize > 0 && stockForSize <= 5) {
                      return <span className="text-xs text-[#AD000F] font-medium">Only {stockForSize} left in size {selectedSize}!</span>;
                    }
                    return null;
                  })()}
                </div>
            ...
            <div className="mt-0 text-neutralDark/70">
              {[
                ...
                <div key={item.id} className="border-b border-neutral-300/70 pb-2">
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-300 ${accordionOpen === item.id
